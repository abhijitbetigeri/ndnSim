...
//Vasu: We need to check if the tag is already set leave it and if not
// Set it. Actually without this even the code works as this is redundant setting
  
  FwCacheHitTag cacheHit;
  bool tagExists_cache = packet->RemovePacketTag (cacheHit);
  if (tagExists_cache)
    {
      cacheHit.CacheHitSet ();
      packet->AddPacketTag (cacheHit);
    }
