...
#include "ns3/ndnSIM/utils/ndn-fw-cache-hit-tag.h"
...
//Vasu: Code to add the Packet tag for the cache hit entry
  FwCacheHitTag cacheHitTag;
  interest->GetPayload ()->AddPacketTag (cacheHitTag);
...
Consumer::OnData (Ptr<const Data> data)
{
  ...
  NS_LOG_INFO ("> Vasudevan hopcount for " << hopCount);

//Vasu: The code changes made to extract the cahce hit entry in the packet and send it 
// to the logger entry 
  int cacheHit = 0;
  FwCacheHitTag cacheHitTag;
  if (data->GetPayload ()->PeekPacketTag (cacheHitTag))
    {
      cacheHit = cacheHitTag.Get ();
    }
  NS_LOG_INFO ("> CacheHit Details: " << cacheHit);

  SeqTimeoutsContainer::iterator entry = m_seqLastDelay.find (seq);
  if (entry != m_seqLastDelay.end ())
    {
//Adding new parameter - cacheHit for getting logged 
      m_lastRetransmittedInterestDataDelay (this, seq, Simulator::Now () - entry->time, hopCount, cacheHit);      
    }
  ...
  if (entry != m_seqFullDelay.end ())
    {
//Adding new parameter - cacheHit for getting logged 
      m_firstInterestDataDelay (this, seq, Simulator::Now () - entry->time, m_seqRetxCounts[seq], hopCount, cacheHit);
    }
